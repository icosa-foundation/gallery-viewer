...
import { unzipSync, strFromU8 } from 'three/examples/jsm/libs/fflate.module.min.js';
	...
	constructor( strokes ) {
		...
		for ( const k in strokes ) {
			...
			for ( let i = 3, j = 4, l = positions.length; i < l; i += 3, j += 4 ) {
				...
				vertices.push( vector1.x, vector1.y, - vector1.z, 1.0 );
				vertices.push( vector2.x, vector2.y, - vector2.z, 1.0 );
				vertices.push( vector4.x, vector4.y, - vector4.z, 1.0 );

				vertices.push( vector2.x, vector2.y, - vector2.z, 1.0 );
				vertices.push( vector3.x, vector3.y, - vector3.z, 1.0 );
				vertices.push( vector4.x, vector4.y, - vector4.z, 1.0 );
		...
		this.setAttribute( 'a_color', new BufferAttribute( new Float32Array( colors ), 4 ) );
		this.setAttribute( 'a_texcoord0', new BufferAttribute( new Float32Array( uvs ), 2 ) );
...
const BRUSH_LIST_ARRAY = {
	...
	'8dc4a70c-d558-4efd-a5ed-d4e860f40dc3': 'SplatterSingleSided',
...
const loader = new TextureLoader().setPath( '../src/tiltbrush/' );
...
const shaders = {
	'Light': {
		uniforms: {
			...
			u_MainTex: { value: loader.load( 'brushes/Light-2241cd32-8ba2-48a5-9ee7-2caef7e9ed62/Light-2241cd32-8ba2-48a5-9ee7-2caef7e9ed62-v10.0-MainTex.png',
			function (texture) {
	...
	'Smoke': {
		uniforms: {
			...
			u_MainTex: { value: loader.load( 'brushes/Smoke-70d79cca-b159-4f35-990c-f02193947fe8/Smoke-70d79cca-b159-4f35-990c-f02193947fe8-v10.0-MainTex.png', 
				function (texture) {
					texture.wrapS = RepeatWrapping;
					texture.wrapT = RepeatWrapping;
					texture.flipY = false;
		...
		vertexShader: TiltBrushShaders.Splatter.Vert,
		fragmentShader: TiltBrushShaders.Splatter.Frag,
		side: 2,
		transparent: true,
		depthFunc: 2,
		depthWrite: true,
	...
	'Splatter': {
		uniforms: {
			...
			u_MainTex: { value: loader.load( 'brushes/Splatter-8dc4a70c-d558-4efd-a5ed-d4e860f40dc3/Splatter-8dc4a70c-d558-4efd-a5ed-d4e860f40dc3-v10.0-MainTex.png', 
				function (texture) {
					texture.wrapS = RepeatWrapping;
					texture.wrapT = RepeatWrapping;
					texture.flipY = false;
...
function getMaterial( GUID ) {
	...
	switch ( name ) {
		...
		case 'Smoke':
			return new RawShaderMaterial( shaders.Smoke );
		case 'Splatter':
			return new RawShaderMaterial( shaders.Splatter );
