...
vec4 PositionParticle(
	...
	float corner = mod(float(vertexId), 4.0);
...
#define PARTICLE_VERTEXID (gl_VertexID)
...
vec4 GetParticlePositionLS() {
	return PositionParticle(gl_VertexID, a_position, PARTICLE_CENTER, PARTICLE_ROTATION);
}
// ---------------------------------------------------------------------------------------------- //
// ---------------------------------------------------------------------------------------------- //
